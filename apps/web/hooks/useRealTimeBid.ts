...
// useEffect(() => {
//   if (!auctionId) return;
//   console.log('[realtime] connecting to auctionId:', auctionId);
//   const fetchUser = async (bidder_id: string) => {
//     if (bidderCache.current.has(bidder_id)) {
//       return bidderCache.current.get(bidder_id);
//     }
...
//     const bid = payload.new as Bid;
//     if (bid?.auction_id !== auctionId) return; // 안전 필터
//     const user = bid.bidder_id ? await fetchUser(bid.bidder_id) : undefined;
//     console.log('[realtime] new bid:', { ...bid, user });
//     onNewBid({ ...bid, user });
//   };
...
//   console.log(channel, '[realtime] channel created for auctionId:', auctionId);
